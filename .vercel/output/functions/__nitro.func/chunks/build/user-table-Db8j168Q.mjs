...
const D = (a) => {
  const c = ["S.N", "First Name", "Last Name", "Email", "Designation", "Mobile Number", "User Type", "To Do", "In Progress", "Overdue", "Completed", "Status"], r = a.map((s, y) => {
    var _a, _b, _c, _d;
    return [(y + 1).toString(), s.fname || "", s.lname || "", s.email || "", s.designation || "", s.phone_number || "", s.user_type || "", ((_a = s.todo_count) == null ? void 0 : _a.toString()) || "0", ((_b = s.in_progress_count) == null ? void 0 : _b.toString()) || "0", ((_c = s.overdue_count) == null ? void 0 : _c.toString()) || "0", ((_d = s.completed_count) == null ? void 0 : _d.toString()) || "0", s.status === "active" ? "Active" : "Inactive"];
...
`), n = new Blob([o], { type: "text/csv;charset=utf-8;" }), i = document.createElement("a"), d = "users.csv";
  ...
  const [c, r, o, n, i, d, s] = a, y = x.get("access_token");
  let g = `https://dev-api-tm.labsquire.com/v3.0/users/status-count?page=${r}&page_size=${o}&active=${n === "active" ? "true" : "false"}`;
  i && (g += `&user_type=${i}`), d && (g += `&search_string=${d}`), s && (g += `&search_email=${s.toLowercase()}`);
  const v = await fetch(g, { headers: { Authorization: `Bearer ${y}` } });
  if (!v.ok) throw new Error("Failed to fetch users");
  return v.json();
}, q = async (a) => {
