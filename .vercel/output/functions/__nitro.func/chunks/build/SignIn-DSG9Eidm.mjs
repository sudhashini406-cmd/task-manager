...
}, _ = k(), u = new Store(_), I = (r) => {
  u.setState((a) => ({ ...a, ...r }));
}, K = () => {
  const r = "https://dev-api-tm.labsquire.com/v3.0/auth/login", [a, p] = useState(false), [n, l] = useState(""), g = useNavigate(), { register: m, handleSubmit: f, setValue: L, getValues: P, formState: { errors: i } } = useForm(), { mutate: h, isLoading: c } = useMutation({ mutationKey: ["login"], mutationFn: async (t) => {
    ...
    x.set("access_token", s, { expires: 7, secure: false }), I({ user: { email: t.data.user_details.email, token: s, role: t.data.user_details.user_type } }), g({ to: "/projects/project-table" });
  }, onError: (t) => {
    l(t.message);
  } });
...
export { K as default };
//# sourceMappingURL=SignIn-DSG9Eidm.mjs.map
